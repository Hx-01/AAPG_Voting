...
import 'package:aapg_voting_app/ogictest/test_start.dart';
import 'dart:async';
import 'package:firebase_database/firebase_database.dart';
import 'package:aapg_voting_app/model/test.dart';
import 'seventh_question.dart';

class sixthQuestion extends StatefulWidget {
  final String nameFromTextField;

  sixthQuestion({Key key, this.nameFromTextField}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _sixthQuestionState();
  }
}
...
class _sixthQuestionState extends State<sixthQuestion> {
  List<String> getListElements() {
    ...
    list.add('High Loss of Circulation rate');
    list.add('Narrow Mud Weight window');
    list.add('Poor ROP');
    list.add('Reservoirs sensitive to Formation Damage caused by drilling operations');
    list.add('All Above');
    return list;
  }
                ...
                ListView(
                  children: <Widget>[
                    ...
                    Container(
                      ...
                      child: Text(
                        'Q6:In Engineering process what are the good candidate for candidates screening:',
  ...
  void _updateAnswer(BuildContext context, int position) {
    ...
    testRef.child('$showName').update({'answer6': finalAnswer});
  }
