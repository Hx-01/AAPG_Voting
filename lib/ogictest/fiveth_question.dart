...
import 'sixth_question.dart';

class FivethQuestion extends StatefulWidget {
  final String nameFromTextField;

  FivethQuestion({Key key, this.nameFromTextField}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _FivethQuestionState();
  }
}

final testRef = FirebaseDatabase.instance.reference().child("Test");

class _FivethQuestionState extends State<FivethQuestion> {
  List<String> getListElements() {
    List<String> list = List<String>();
    list.add('Reduce Formation Damage');
    list.add('Limit or Avoid Lost Circulations');
    list.add('Avoid Differential Sticking');
    list.add('All above');
    return list;
  }
  ...
  void _updateAnswer(BuildContext context, int position) {
    ...
    testRef.child('$showName').update({'answer5': finalAnswer});
  }
